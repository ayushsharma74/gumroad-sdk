...
export function createOfferCommand(): Command {
        ...
        .action(
            async (options: {
                ...
                try {
                    ...
                    const input: Parameters<typeof client.offers.create>[0] = {
                        product_id: options.productId,
                        name,
                        amount_off: amountOff,
                    };
                    if (options.offerType !== undefined) input.offer_type = options.offerType as "cents" | "percent";
                    if (options.maxPurchaseCount !== undefined) input.max_purchase_count = parseInt(options.maxPurchaseCount, 10);
                    const offer = await client.offers.create(input);

                    output(offer, options, (o: Offer) => {
